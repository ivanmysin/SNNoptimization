...
with tf.GradientTape() as tape:
    ...
    gamma_s_envelope = 0.1 * exp(2 * cos(theta_phi - peak_gamma_on_theta_phase) )
    gamma_s =  gamma_s_envelope * cos( 2*np.pi*t*omega_gamma_s)
    #gamma_m = 0.01 * exp(2 * cos(theta_phi) ) * cos( 2*np.pi*t*90)
    #######################################################
    omega0 = 12.0
    s = omega0 / omega_gamma_s
    t_morlet =  tf.linspace(-2, 2, 500)
    ...
    wavelet_spetum = 0.1*sqrt(wavelet_spetum_real**2 + wavelet_spetum_imag**2)
