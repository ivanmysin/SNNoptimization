...
eqs = '''
dV/dt = (gK*n**4*(EK - V) + gL*(EL - V) + Iapp )/Cm  + sigma*xi/sqrt(tau)  : volt (unless refractory)
...
neuron = NeuronGroup(N, eqs, threshold='V > VT', \
                     reset='V = -40*mV; n = 0.45; VT = -30*mV', refractory=5*ms, method='heun') # , method='exponential_euler'
neuron.V = EL
neuron.VT = -50*mV
neuron.n = 0
...
spike_rate, bins = np.histogram(spikemon.t/ms, 200, density=False)
spike_rate = spike_rate / (1000*N*defaultclock.dt)
#_ = hist(spikemon.t/ms, 100, histtype='stepfilled', facecolor='k', weights=list(ones(len(spikemon))/(1000*N*defaultclock.dt)))
#plot(M.t/ms, M[0].V/mV)
plot(bins[1:], spike_rate)
show()
