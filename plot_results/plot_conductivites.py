...
path = '/home/ivan/Data/Opt_res/!!!LIF_solution_310.hdf5'
path = '/home/ivan/Data/phase_relations/solution_947.hdf5'
...
for idx in range(len(params_net["params_synapses"])):
    ...
    ax.plot(t, g_syn, linestyle="-", label=syn["pre_name"], color=color, linewidth=5)


sine = 0.5 * (np.cos(2 * np.pi * t * 7.0) + 1)
axes[0].plot(t, exc_g, linestyle="-", label = "sum exc", color='orange', linewidth=2)
axes[1].plot(t, inh_g, linestyle="-", label = "sum inh", color='magenta', linewidth=2)
...
print(np.std( inh_exc_relation[10000:]) )
