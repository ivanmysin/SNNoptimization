...
def get_batch():
    ...
    batch_y0 = tf.cast(batch_y0, tf.float64)
    batch_t = tf.cast(batch_t, tf.float64)
    batch_y = tf.cast(batch_y, tf.float64)
...
if __name__ == '__main__':
    ...
    with tf.device(device):
        ...
        lr = 1e-3
        optimizer = tf.keras.optimizers.RMSprop(lr) # tf.train.RMSPropOptimizer(lr)
