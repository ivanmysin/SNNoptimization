...
const size_t _num__source_gK_neurongroup_n = 1;
...
const double gbarKdr = 0.009000000000000001;
    ...
    _dynamic_array_statemonitor_2_gK.resize(_new_size, _num_indices);
    ...
    for (int _i = 0; _i < (int)_num_indices; _i++)
    {
        ...
        const double __source_gK_neurongroup_n = _ptr_array_neurongroup_n[_idx];
        const double _source_gK = gbarKdr * (_brian_pow(__source_gK_neurongroup_n, 4));
        const double _to_record_gK = _source_gK;


        _dynamic_array_statemonitor_2_gK(_new_size-1, _i) = _to_record_gK;
    }
